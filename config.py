...
def argparser(is_train=True):
    ...
    parser.add_argument('--dataset', type=str, default='car', choices=[
        'car', 'chair', 'kitti', 'synthia', 'megacar', 'megachair',
        'megaplane', 'miniplane'])
    ...
    if config.dataset in ['car', 'chair', 'megacar', 'megachair', 'megaplane',
            'miniplane']:
    ...
    dataset_train, dataset_test, datagenerator_test = \
        dataset.create_default_splits(config.num_input, config.dataset, batch_size=config.batch_size)
